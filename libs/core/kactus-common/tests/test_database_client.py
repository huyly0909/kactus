...
class TestDatabaseClient:
    ...
    def test_insert_data_basic(self, client, sample_table, sample_data):
        ...
        result = client.execute(
            f"SELECT COUNT(*) as count FROM {sample_table.name}"
        ).fetchone()
        assert result[0] == 3
    ...
    def test_replace_strategy(self, client, sample_data):
        ...
        names = client.execute(
            f"SELECT name FROM {table.name} ORDER BY name"
        ).fetchall()
    ...
    def test_upsert_strategy_single_primary_key(self, client):
        ...
        upsert_data = pd.DataFrame(
            {"id": [1, 4], "name": ["Alice Updated", "David"], "score": [95, 88]}
        )
        ...
        alice_score = client.execute(
            f"SELECT score FROM {table.name} WHERE id = 1"
        ).fetchone()
        assert alice_score[0] == 95

        # Check that David was added
        david_exists = client.execute(
            f"SELECT name FROM {table.name} WHERE id = 4"
        ).fetchone()
    ...
    def test_upsert_strategy_multiple_primary_keys(self, client):
        ...
        # Insert initial data
        initial_data = pd.DataFrame(
            {"user_id": [1, 1, 2], "product_id": [100, 101, 100], "rating": [4, 5, 3]}
        )
        client.update_table(table, initial_data)

        # Upsert: update (1,100) and add (2,101)
        upsert_data = pd.DataFrame(
            {"user_id": [1, 2], "product_id": [100, 101], "rating": [5, 4]}
        )
        client.update_table(table, upsert_data)
        ...
        rating = client.execute(
            f"SELECT rating FROM {table.name} WHERE user_id = 1 AND product_id = 100"
        ).fetchone()
        assert rating[0] == 5
    ...
    def test_insert_overwrite_strategy(self, client):
        ...
        jan_02_data = client.execute(
            f"SELECT sales FROM {table.name} WHERE date = '2024-01-02'"
        ).fetchone()
        assert jan_02_data[0] == 200

        # Check that 2024-01-01 data was overwritten
        jan_01_products = client.execute(
            f"SELECT product FROM {table.name} WHERE date = '2024-01-01' ORDER BY product"
        ).fetchall()
    ...
    def test_empty_data_handling(self, client, sample_table):
        ...
        for strategy in [
            ...
            UpdateStrategy.INSERT_OVERWRITE,
        ]:
            table = sample_table.model_copy()
    ...
    def test_upsert_missing_primary_key_columns(self, client):
        ...
        with pytest.raises(
            ValueError, match="Primary key columns .* not found in data"
        ):
    ...
    def test_context_manager_connection(self, temp_db_path):
        ...
        with client.get_connection() as conn2:
            ...
            result = conn2.execute(
                "SELECT table_name FROM information_schema.tables WHERE table_name='test_ctx'"
            )
            # A fresh connection sees the table the previous one created.
