...
Everything here is served from DuckDB (OLAP), written by the kactus-data ETL.
Reads are session-authenticated but not project-scoped: market data is public
reference data, not user-owned.
"""
...
async def list_gold_prices(
    ...
    return await MarketService.list_gold(get_olap_storage(), codes=code)
...
async def search_stocks(
    ...
    return await MarketService.search_stocks(get_olap_storage(), q=q, limit=limit)
...
async def list_stock_quotes(
    ...
    return await MarketService.list_quotes(
        get_olap_storage(), symbols=symbol, limit=limit
    )
...
async def get_stock(symbol: str) -> StockDetailSchema:
    ...
    detail = await MarketService.get_stock(get_olap_storage(), symbol)
    if detail is None:
...
async def list_stock_news(
    ...
    return await MarketService.list_news(get_olap_storage(), symbol, limit=limit)
