"""Gold API schemas — read models over the OLAP (DuckDB) gold tables.
...
from kactus_common.schemas import AwareUTCDatetime, BaseSchema, FancyDecimal, FancyInt

from .schedule import ScheduleTimezone
...
class GoldScheduleSchema(BaseSchema):
    """One gold series' data-availability schedule, echoing the series identity.

    ``expected_weekdays`` uses Mon=0 … Sun=6; ``timezone`` is the market's
    operating zone the weekdays/holidays are interpreted in (UTC | Asia/Ho_Chi_Minh).
    """
